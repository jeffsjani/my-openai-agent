...
export async function runWorkflow(workflow) {
  return await withTrace("CHAPTER WORKER v2", async () => {
    ...
    if (parsedInput?.debug_real_node3_after_mock12 === true) {
      const mockNode2Packet = {
        story_run_id: parsedInput?.story_run_id ?? null,
        project_id: parsedInput?.project_id ?? null,
        chapter_worker_version: parsedInput?.chapter_worker_version ?? null,
        chapter_context: parsedInput?.chapter_context ?? null,
        ...
        locked_draft_priorities_packet: {
          ...
          locked_priorities_shorthand: "Protect contamination and oxygen."
        },
        character_constellation_packet: {
          source_section_number: 7,
          constellation_mandate: "Callan and Elise are dangerous complements.",
          ...
          relationship_pressure_lines: ["Attraction collides with operational secrecy."],
          attraction_conflict_axes: ["Recognition fused to danger."],
          trust_fault_lines: ["Immediate mutual recognition before trust is safe."],
          constellation_shorthand: "Dangerous complements."
        },
        structural_spine_packet: {
          source_section_number: 11,
          current_unit_spine_function: "Romantic ignition before founding wound.",
          upstream_dependencies: ["Athens myth display and cracked-watch residue."],
          downstream_setup_payoffs: ["Denied payoff primes Beirut Mission reunion."],
          escalation_logic: ["Myth under fire -> bar recognition -> mission wound."],
          spine_shorthand: "Romantic ignition under hidden urgency."
        },
        setpiece_symbol_architecture_packet: {
          source_section_number: 13,
          setpiece_mandate: "Respectable surface with hidden operational pressure.",
          chapter_relevant_setpieces: ["Beirut bar selection and near-kiss denial."],
          object_symbol_rules: ["Mirrored amber glass is active atmosphere and symbol."],
          visual_escalation_cues: ["Exit line, mirrored surfaces, unfinished look."],
          setpiece_failure_signs: ["Generic flirtation without operational consequence."],
          architecture_shorthand: "Seduction as pressure geometry."
        },
        world_setting_palette_packet: {
          source_section_number: 8,
          setting_palette_rules: ["Contemporary, politically plausible, sensorially exact."],
          city_specific_texture_rules: ["Beirut glows and ruptures.", "Mirrored amber light."],
          environmental_pressure_cues: ["Bar edges, exits, corridor pull."],
          institutional_surface_rules: ["Elegant hotel calm masking hidden urgency."],
          palette_shorthand: "Mirrored amber Beirut under hidden pressure."
        },
        thematic_moral_architecture_packet: {
          source_section_number: 9,
          thematic_mandate: "Truth, intimacy, and leverage are fused.",
          central_thematic_questions: ["What kind of truth can a room survive?"],
          moral_pressure_lines: ["Attraction and duty contaminate each other."],
          private_public_cost_rules: ["Private recognition opens into public consequence."],
          contradiction_and_complicity_rules: ["Immediate intimacy already sits inside hidden systems."],
          thematic_shorthand: "Contaminated intimacy under managed truth."
        },
        signature_verbal_deployment_packet: {
          source_section_number: 21,
          signature_language_rules: ["Callan sounds calm and plain.", "Elise sounds deliberate and costly."],
          recurring_phrase_motifs: ["What people can bear."],
          metaphor_and_comparison_rules: ["Precision over ornament."],
          verbal_do_not_use: ["Pseudo-profound generic spy talk."],
          signature_shorthand: "Pressure-true signature voice."
        },
        research_authenticity_packet: {
          source_section_number: 18,
          authenticity_mandate: "Grounded in recognizable allied-security culture and plausible urban texture.",
          procedural_truth_rules: ["Hospitality and diplomatic surfaces must feel real."],
          terminology_and_register_rules: ["Avoid fake technical swagger."],
          domain_specific_constraints: ["Keep Beirut hotel-bar dynamics credible and specific."],
          realism_do_not_fake: ["Do not invent implausible operator jargon."],
          authenticity_shorthand: "Plausible, tactile, politically literate."
        },
        prestige_quality_alignment_packet: {
          source_section_number: 4,
          prestige_mandate: "Prestige-level espionage density with immediate commercial legibility.",
          quality_benchmarks: ["Elegant but emotionally charged.", "Trailer-legible without flattening intelligence."],
          anti_generic_failures: ["Generic spy cool.", "Empty glamour."],
          audience_promise_rules: ["The couple must be thrilling before the plot weaponizes them."],
          prestige_shorthand: "Prestige espionage-romance with commercial propulsion."
        },
        unit_contracts: [
          {
      ...
      const tempHistory = [
        {
          role: "user",
          content: [{ type: "input_text", text: JSON.stringify(mockNode2Packet) }]
        }
      ];
...
function getEnv(name) {
  try {
    ...
    // Ignore Netlify.env lookup errors and fall back to process.env.
  }

  return process.env?.[name] ?? null;
}

function extractFinalChapterText(workflowResult) {
  if (!workflowResult || typeof workflowResult !== "object") return null;

  return (
    workflowResult.final_chapter_text ??
    workflowResult.final_combined_book_text ??
    workflowResult.drafted_units?.[0]?.drafted_text ??
    workflowResult.result_payload_json?.final_chapter_text ??
    workflowResult.result_payload_json?.final_combined_book_text ??
    null
  );
}

function buildCompletionPayload({ body, result, error }) {
  const completedAt = new Date().toISOString();
  const workflowResult = result?.output_parsed ?? null;
  const workflowResultText = result?.output_text ?? null;

  const storyRunId =
    body?.story_run_id ??
    workflowResult?.story_run_id ??
    null;

  const finalChapterText = extractFinalChapterText(workflowResult);
  ...
  if (body?.debug_stop_after_node2 === true) {
    ...
    const diagnosticText =
      ...
      "status: " + (workflowResult?.status ?? "unknown") + "\n" +
      "story_run_id: " + (storyRunId ?? "") + "\n" +
      "target_units_requested: " + JSON.stringify(workflowResult?.target_units_requested ?? []) + "\n" +
      "unit_contracts_count: " + (Array.isArray(workflowResult?.unit_contracts) ? workflowResult.unit_contracts.length : 0) + "\n" +
      "missing_required_inputs: " + JSON.stringify(workflowResult?.missing_required_inputs ?? []) + "\n" +
      "blocked_reasons: " + JSON.stringify(workflowResult?.blocked_reasons ?? []) + "\n" +
      "next_node: " + (workflowResult?.next_node ?? "");
...
async function postCompletion(payload) {
  const completeUrl =
    getEnv("BUILDSHIP_COMPLETE_URL") ||
    getEnv("BUILDSHIP_STORY_RUN_COMPLETE_URL") ||
    null;

  if (!completeUrl) {
    console.error("[agent-background] Missing BUILDSHIP_COMPLETE_URL");
    console.error("[agent-background] Completion payload was:", JSON.stringify(payload).slice(0, 4000));
    return {
      ok: false,
      error: "Missing BUILDSHIP_COMPLETE_URL"
    };
  }

  const sharedSecret = getEnv("AGENT_SHARED_SECRET");
  ...
  if (sharedSecret) {
    headers["x-agent-secret"] = sharedSecret;
  }
  ...
  console.log("[agent-background] completion callback status", response.status);
  console.log("[agent-background] completion callback elapsed_ms", Date.now() - startedAt);
  console.log("[agent-background] completion callback preview", responseText.slice(0, 1000));
...
export default async (req, context) => {
  ...
  try {
    if (req.method !== "POST") {
      console.error("[agent-background] Method not allowed", req.method);
      return;
    }
    ...
    if (sharedSecret) {
      ...
      if (providedSecret !== sharedSecret) {
        console.error("[agent-background] Unauthorized request");
        return;
      }
    }
    ...
    console.log("[agent-background] invoked_at", invokedAt);
    console.log("[agent-background] received_keys", Object.keys(body || {}));
    console.log("[agent-background] story_run_id", body?.story_run_id ?? null);
    console.log("[agent-background] worker_mode", body?.worker_mode ?? null);
    ...
    if (body?.debug_env === true) {
      ...
      await postCompletion({
        ok: true,
        status: "completed",
        story_run_id: body?.story_run_id ?? "debug_env",
        completed_at: new Date().toISOString(),
        result_payload_json: {
          ok: true,
          mode: "debug_env",
    ...
    if (
      ...
      await postCompletion({
        ok: true,
        status: "completed",
        story_run_id: body?.story_run_id ?? "background_ping_test",
        completed_at: new Date().toISOString(),
        result_payload_json: {
          ok: true,
    ...
    if (!inputPayload || !inputPayload.trim()) {
      await postCompletion({
        ...
        completed_at: new Date().toISOString(),
        error_message: "Missing request payload",
        result_payload_json: {
          ok: false,
          error: "Missing request payload",
          error_type: "missing_request_payload"
        },
        final_chapter_text: null,
        final_combined_book_text: null
      });
    ...
    await postCompletion(completionPayload);
  } catch (error) {
    ...
    await postCompletion(failurePayload);
  }
};

// Background functions are invoked by the default Netlify function URL:
// /.netlify/functions/agent-background
//
// Keep this file named exactly:
// netlify/functions/agent-background.mjs
//
