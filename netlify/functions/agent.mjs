...
/*
===============================================================================
STEP A — PASTE YOUR GENERATED AGENT BUILDER SDK BLOCK BELOW
-------------------------------------------------------------------------------
Paste ALL of your generated SDK code here, EXCEPT these two parts:

1. REMOVE this line from the generated code before pasting:
   type WorkflowInput = { input_as_text: string };

2. REMOVE the generated runWorkflow function block before pasting:
   export const runWorkflow = async (workflow: WorkflowInput) => { ... }

You SHOULD keep and paste:
- const fileSearch = ...
- all Zod schemas
- all Agent declarations
...
/* REMOVED generated runWorkflow */
  return await withTrace("CHAPTER WORKER v2", async () => {
    const state = {
      rewrite_cycle_completed: "0",
      max_enhancement_cycles: "3",
      remaining_rewrite_cycles: "3",
      max_polish_cycles: "2",
      polish_cycle_completed: "0",
      remaining_polish_cycles: null,
      inputoutput_parsedpolish_cycle_completed: null,
      inputoutput_parsedmax_polish_cycles: null,
      inputoutput_parsedremaining_polish_cycles: null,
      inputoutput_parsedstatus: null,
      inputoutput_parsedrewrite_cycle_completed: null,
      inputoutput_parsedmax_enhancement_cycles: null,
      inputoutput_parsedremaining_rewrite_cycles: null,
    ...
    const node1IntakeScopeLockCanonBasisAndStoreRoutingResultTemp = await runner.run(
      node1IntakeScopeLockCanonBasisAndStoreRouting,
      [
        ...conversationHistory
      ]
    ...
    if (!node1IntakeScopeLockCanonBasisAndStoreRoutingResultTemp.finalOutput) {
        throw new Error("Agent result is undefined");
    }

    const node1IntakeScopeLockCanonBasisAndStoreRoutingResult = {
      output_text: JSON.stringify(node1IntakeScopeLockCanonBasisAndStoreRoutingResultTemp.finalOutput),
      output_parsed: node1IntakeScopeLockCanonBasisAndStoreRoutingResultTemp.finalOutput
    };
    const node2UnitContractBuilderResultTemp = await runner.run(
      node2UnitContractBuilder,
      [
        ...conversationHistory
      ]
    );
    conversationHistory.push(...node2UnitContractBuilderResultTemp.newItems.map((item) => item.rawItem));

    if (!node2UnitContractBuilderResultTemp.finalOutput) {
        throw new Error("Agent result is undefined");
    }

    const node2UnitContractBuilderResult = {
      output_text: JSON.stringify(node2UnitContractBuilderResultTemp.finalOutput),
      output_parsed: node2UnitContractBuilderResultTemp.finalOutput
    };
    const node3ChapterDrafterResultTemp = await runner.run(
      node3ChapterDrafter,
      [
        ...conversationHistory
      ]
    );
    conversationHistory.push(...node3ChapterDrafterResultTemp.newItems.map((item) => item.rawItem));

    if (!node3ChapterDrafterResultTemp.finalOutput) {
        throw new Error("Agent result is undefined");
    }

    const node3ChapterDrafterResult = {
      output_text: JSON.stringify(node3ChapterDrafterResultTemp.finalOutput),
      output_parsed: node3ChapterDrafterResultTemp.finalOutput
    };
    state.rewrite_cycle_completed = 0;
    state.max_enhancement_cycles = 4;
    state.remaining_rewrite_cycles = 4;
    while (int(state.remaining_rewrite_cycles) > 0) {
      const node4aUpstreamDraftGateResultTemp = await runner.run(
        node4aUpstreamDraftGate,
        [
          ...conversationHistory
        ]
      );
      conversationHistory.push(...node4aUpstreamDraftGateResultTemp.newItems.map((item) => item.rawItem));

      if (!node4aUpstreamDraftGateResultTemp.finalOutput) {
          throw new Error("Agent result is undefined");
      }

      const node4aUpstreamDraftGateResult = {
        output_text: JSON.stringify(node4aUpstreamDraftGateResultTemp.finalOutput),
        output_parsed: node4aUpstreamDraftGateResultTemp.finalOutput
      };
      if (node4aUpstreamDraftGateResult.output_parsed.status == "ready") {
        const node4bEnhancementEvaluatorResultTemp = await runner.run(
          node4bEnhancementEvaluator,
          [
            ...conversationHistory
          ]
        );
        conversationHistory.push(...node4bEnhancementEvaluatorResultTemp.newItems.map((item) => item.rawItem));

        if (!node4bEnhancementEvaluatorResultTemp.finalOutput) {
            throw new Error("Agent result is undefined");
        }

        const node4bEnhancementEvaluatorResult = {
          output_text: JSON.stringify(node4bEnhancementEvaluatorResultTemp.finalOutput),
          output_parsed: node4bEnhancementEvaluatorResultTemp.finalOutput
        };
        if (node4bEnhancementEvaluatorResult.output_parsed.status == "ready") {
          const node5ChapterRewriterResultTemp = await runner.run(
            node5ChapterRewriter,
            [
              ...conversationHistory
            ]
          );
          conversationHistory.push(...node5ChapterRewriterResultTemp.newItems.map((item) => item.rawItem));

          if (!node5ChapterRewriterResultTemp.finalOutput) {
              throw new Error("Agent result is undefined");
          }

          const node5ChapterRewriterResult = {
            output_text: JSON.stringify(node5ChapterRewriterResultTemp.finalOutput),
            output_parsed: node5ChapterRewriterResultTemp.finalOutput
          };
          state.rewrite_cycle_completed = node5ChapterRewriterResult.output_parsed.rewrite_cycle_completed;
          state.max_enhancement_cycles = node5ChapterRewriterResult.output_parsed.max_enhancement_cycles;
          state.remaining_rewrite_cycles = node5ChapterRewriterResult.output_parsed.remaining_rewrite_cycles;
        } else {
          return node4bEnhancementEvaluatorResult;
        }
      } else {
        return node4aUpstreamDraftGateResult;
      }
    }
    state.max_polish_cycles = 2;
    state.polish_cycle_completed = 0;
    state.remaining_polish_cycles = 2;
    while (int(state.remaining_polish_cycles) > 0) {
      const node6PolishReadyGateResultTemp = await runner.run(
        node6PolishReadyGate,
        [
          ...conversationHistory
        ]
      );
      conversationHistory.push(...node6PolishReadyGateResultTemp.newItems.map((item) => item.rawItem));

      if (!node6PolishReadyGateResultTemp.finalOutput) {
          throw new Error("Agent result is undefined");
      }

      const node6PolishReadyGateResult = {
        output_text: JSON.stringify(node6PolishReadyGateResultTemp.finalOutput),
        output_parsed: node6PolishReadyGateResultTemp.finalOutput
      };
      if (node6PolishReadyGateResult.output_parsed.status == "ready") {
        const node7PolishEvaluatorResultTemp = await runner.run(
          node7PolishEvaluator,
          [
            ...conversationHistory
          ]
        );
        conversationHistory.push(...node7PolishEvaluatorResultTemp.newItems.map((item) => item.rawItem));

        if (!node7PolishEvaluatorResultTemp.finalOutput) {
            throw new Error("Agent result is undefined");
        }

        const node7PolishEvaluatorResult = {
          output_text: JSON.stringify(node7PolishEvaluatorResultTemp.finalOutput),
          output_parsed: node7PolishEvaluatorResultTemp.finalOutput
        };
        if (node7PolishEvaluatorResult.output_parsed.status == "ready") {
          const node8PolishRewriterResultTemp = await runner.run(
            node8PolishRewriter,
            [
              ...conversationHistory
            ]
          );
          conversationHistory.push(...node8PolishRewriterResultTemp.newItems.map((item) => item.rawItem));

          if (!node8PolishRewriterResultTemp.finalOutput) {
              throw new Error("Agent result is undefined");
          }

          const node8PolishRewriterResult = {
            output_text: JSON.stringify(node8PolishRewriterResultTemp.finalOutput),
            output_parsed: node8PolishRewriterResultTemp.finalOutput
          };
          if (node8PolishRewriterResult.output_parsed.status == "ready") {
            state.polish_cycle_completed = 1;
            state.max_polish_cycles = 1;
            state.remaining_polish_cycles = 0;
          } else {
            return node8PolishRewriterResult;
          }
        } else {
          return node7PolishEvaluatorResult;
        }
      } else {
        return node6PolishReadyGateResult;
      }
    }
    const node9FinalChapterOutputResultTemp = await runner.run(
      node9FinalChapterOutput,
      [
        ...conversationHistory
      ]
    );
    conversationHistory.push(...node9FinalChapterOutputResultTemp.newItems.map((item) => item.rawItem));

    if (!node9FinalChapterOutputResultTemp.finalOutput) {
        throw new Error("Agent result is undefined");
    }

    const node9FinalChapterOutputResult = {
      output_text: JSON.stringify(node9FinalChapterOutputResultTemp.finalOutput),
      output_parsed: node9FinalChapterOutputResultTemp.finalOutput
    };
    const node10StoryOrchestratorHandoffPackagerResultTemp = await runner.run(
      node10StoryOrchestratorHandoffPackager,
      [
        ...conversationHistory
      ]
    );
    conversationHistory.push(...node10StoryOrchestratorHandoffPackagerResultTemp.newItems.map((item) => item.rawItem));

    if (!node10StoryOrchestratorHandoffPackagerResultTemp.finalOutput) {
        throw new Error("Agent result is undefined");
    }

    const node10StoryOrchestratorHandoffPackagerResult = {
      output_text: JSON.stringify(node10StoryOrchestratorHandoffPackagerResultTemp.finalOutput),
      output_parsed: node10StoryOrchestratorHandoffPackagerResultTemp.finalOutput
    };
  });
}
...
/*
===============================================================================
STEP B — DO NOT CHANGE THE CODE BELOW UNLESS YOU KNOW EXACTLY WHY
===============================================================================
*/
